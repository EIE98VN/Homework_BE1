...
public class ScheduledEcomTransaction {
  ...
  @Autowired
  EcomTransactionService ecomTransactionService;

  @Autowired
  SyncContactService syncContactService;

  @Autowired
  MarolService marolService;

  @Autowired
  MagentoService magentoService;

  @Autowired
  EcomCodeService ecomCodeService;
  ...
  private void updateExpiredTransaction() {
    try {
      ...
      log.info("(updateExpiredTransaction) startUpdateTime: {} - endUpdateTime: {}",
          startTime, endTime);
      List<String> listStatus = Arrays.asList(PaymentStatus.CREATED, PaymentStatus.PENDING);

      // Get expired orders
      List<EcomTransaction> orderList = ecomTransactionService.findOrdersByStatusIn(
          startTime, endTime, listStatus);
  ...
  private void syncExpiredTransaction(EcomTransaction transaction) {
    ...
    if(orderMagento == null){
      log.error("etl_internal_error= ERROR_CANNOT_GET_ORDER_MAGENTO; chargeId: {}",
          order.getChargeId());
      return ;
    }
    UserMagento userMagento = magentoService.findUserById(order.getUserId());
    if(userMagento == null){
      log.error("etl_internal_error= ERROR_CANNOT_GET_USER_MAGENTO; chargeId: {}",
          order.getChargeId());
      return ;
    }
  ...
  private void createC3convert(OrderMagento orderMagento, UserMagento userMagento, String method) {
    ...
    itemMagentoList.forEach(itemMagento -> {
